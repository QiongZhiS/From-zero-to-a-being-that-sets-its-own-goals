...
>
> 项目创建：2026-08 ｜ 状态：**三个阶段完成 + B0 规模测试 + 交叉验证第一轮**（20 个实验 + 17 篇文档）
> 第三阶段归档：**延续（内生连续性）**——docs/14 理论 + docs/15 小结（SEED-18/19）
> B0 结论：**规模不是变量**——机制结论（分工/竞争排斥）在 300~5000 个体下不变
> 交叉验证（docs/17）：对方蒸馏实证证实信号质量预测——self-distill -16.5pp vs human-label +2.5pp
> 下一步：路线 A 建议闭环给对方（蒸馏+验证层）；或路线 C 信号质量独立文章
