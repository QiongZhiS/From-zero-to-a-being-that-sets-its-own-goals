"""
SEED-16: Division of labor -- does specialization emerge?

World has two food regions with different economics:
  A-zone: sparse but rich (24 food x 80 energy)
  B-zone: dense but poor  (96 food x 30 energy)
Agents carry a heritable `preference` (0 = specialize A, 1 = specialize B).
Middle preferences keep switching zones (travel cost), extremes stay put
(efficient). Selection should push preference to a BIMODAL distribution --
division of labor emerging from economics, not design.

Regimes:
  uniform   preference fixed (control: no evolution)
  genetic   preference evolves by mutation
  cultural  preference evolves + imitation of most successful agent

Measure: preference distribution (bimodality = division of labor),
births/starved (group efficiency).

Run:  python seed16.py [--mode cultural] [--ticks 8000] [--seed 42]
"""
...
B_N = 96
B_ENERGY = 30
A_X_MAX = 24      # A zone: x in [0, A_X_MAX]
B_X_MIN = 40      # B zone: x in [B_X_MIN, SIZE)
...
class World:
    ...
    def nearest_food_in_zone(self, x, y, zone):
        best = None
        bd = 10 ** 9
        for f in self.foods:
...
def choose_zone(agent, rng):
    """Pheno preference -> zone. Middle preferences switch often (travel
    cost), extreme preferences stay put (efficient)."""
    return "B" if rng.random() < agent.pheno_pref else "A"


def act(agent, world):
    if world.food_at(agent.x, agent.y) is not None:
        return "GATHER"
    zone = choose_zone(agent, world.rng)
    delta = world.nearest_food_in_zone(agent.x, agent.y, zone)
    if delta is None:
        # fallback: any food
        delta = (world.nearest_food_in_zone(agent.x, agent.y, "A")
                 or world.nearest_food_in_zone(agent.x, agent.y, "B"))
        if delta is None:
...
def simulate(ticks=8000, mode="cultural", seed=42, report_every=800):
    ...
    rng = world.rng
    if mode == "uniform":
        prefs = [0.0 if i % 2 == 0 else 1.0 for i in range(50)]
    else:
        prefs = [rng.random() for _ in range(50)]
    agents = [Agent(rng.randrange(SIZE), rng.randrange(SIZE),
                    INIT_ENERGY, p, p) for p in prefs]
    ...
    for t in range(ticks):
        ...
        for a in list(agents):
            if a.alive and a.energy >= SPLIT_ENERGY and len(agents) < MAX_POP:
                ...
                agents.append(Agent(
                    ...
                    a.energy / 2.0, gp, gp, a.generation + 1))
                a.energy /= 2.0
                stats["births"] += 1
        ...
        if (t + 1) % report_every == 0:
            ...
            print(f"t={t+1:5d}  pop={len(agents):4d}  mean_pref={pref:.3f}  "
                  f"A-spec={lo:.2f}  mid={mid:.2f}  B-spec={hi:.2f}")

    print(f"\nfinal({mode}): pop={len(agents)} births={stats['births']} "
          f"starved={stats['starved']}")
    if agents:
        ps = sorted(a.pheno_pref for a in agents)
        ...
        print(f"  preference: A-spec={lo}  mid={mid}  B-spec={hi}  "
              f"range=[{ps[0]:.2f},{ps[-1]:.2f}]")
    return stats


def main():
    p = argparse.ArgumentParser(description="SEED-16 division of labor")
    p.add_argument("--mode", choices=["uniform", "genetic", "cultural"],
                   default="cultural")
